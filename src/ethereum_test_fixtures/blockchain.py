...
class BlockchainEngineSyncFixture(BlockchainEngineFixtureCommon):
    ...
    - Both client types are parametrized for comprehensive testing
    """

    format_name: ClassVar[str] = "blockchain_test_engine_sync"
    description: ClassVar[str] = (
        "Tests that generate a blockchain test fixture for Engine API sync testing."
    )
    pre: Alloc
    genesis: FixtureHeader = Field(..., alias="genesisBlockHeader")
    post_state: Alloc | None = Field(None)
