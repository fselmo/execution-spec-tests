...
class BlockchainTest(BaseTest):
    ...
    def make_hive_fixture(
        ...
        if fixture_format == BlockchainEngineSyncFixture:
        ...
        elif self.verify_sync:
            # For backward compatibility: if verify_sync is set but not using sync fixture format
            if fixture_format in [BlockchainEngineFixture, BlockchainEngineXFixture]:
                ...
                # Most clients require the header to start the sync process, so we create an
                # empty block on top of the last block of the test to send it as new payload
